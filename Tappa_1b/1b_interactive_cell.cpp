...
struct Grid
{
    ...
    Grid (sf::Vector2i cell_num, int mine_num, float cell_size); //MODIFICA: aggiunto il numero di mine poichè anche quello varia in base alla difficoltà
...
struct Game_Panel
{
    ...
    //MODIFICA: aggiunto il numero di mine poichè anche quello varia in base alla difficoltà
...
void handle (const sf::Event::MouseButtonPressed& mouse, State& state)
{
    ...
    if( mouse.button == sf::Mouse::Button::Left){
        ...
        else state.reveal(state.game_panel.grid, state.mouse_cell); // in caso non sia la prima mossa, rivela solo la cella cliccata 
    }
