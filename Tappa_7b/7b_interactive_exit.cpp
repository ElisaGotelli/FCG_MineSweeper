...
void handle_mouse_pressed (const sf::Event::MouseButtonPressed& mouse, sf::RenderWindow& window, State& state)
{
    ...
    if( mouse.button == sf::Mouse::Button::Left){
        ...
        if(state.cp.exit.cb_bounds.contains(mouse_pos)){ //AGGIUNTA: se è stato cliccato il pulsante ESCI neLl Control Panel chiama la funzione che gestisce l'uscita dalla schermata di gioco 
            state.exit(); 
            return; 
        }
