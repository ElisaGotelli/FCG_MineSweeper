...
struct Control_Panel 
{
    ...
    Control_Panel(Border border): 
                                    ...
                                    exit(button_type::exit, {cp_pos.x +cp_size.x/2.f - button_size.x/2.f, pause.cb_pos.y + pause.cb_size.y + control_vertical_displacement + control_gap}, button_size) 
                                    {}
...
{ 
    ...
    Game_Stop ():
                ...
                time(0), 
                new_game_cb(button_type::new_game,{gs_pos.x + gs_size.x/3.f - std_stop_button_width/2.f, gs_pos.y + gs_size.y - stop_gap - std_stop_button_heigth}, {std_stop_button_width, std_stop_button_heigth}), //EX MODIFICA
                ...
                exit_cb(button_type::exit, {gs_pos.x + gs_size.x*2.f/3.f - std_stop_button_width/2.f, gs_pos.y + gs_size.y - stop_gap - std_stop_button_heigth}, {std_stop_button_width, std_stop_button_heigth}) //EX MODIFICA
                {} 
...
void Game_Stop::draw(sf::RenderWindow& window){ 
    ...
    { 
        ...
        new_game_cb.cb_size = exit_cb.cb_size =  {std_stop_button_width*1.5f, std_stop_button_heigth*1.5f};  //ex modifica
        new_game_cb.cb_pos = {gs_pos.x + gs_size.x/3.f - exit_cb.cb_size.x/2.f, gs_pos.y + gs_size.y - stop_gap - std_stop_button_heigth*1.5f}; //EX MODIFICA
        new_game_cb.cb_bounds = {new_game_cb.cb_pos, new_game_cb.cb_size};
        //tolto: new_game_cb.cb_size;  (ex modifica)
        exit_cb.cb_pos = {gs_pos.x + gs_size.x*2.f/3.f - exit_cb.cb_size.x/2.f, new_game_cb.cb_pos.y}; //EX MODIFICA
...
void handle_mouse_pressed (const sf::Event::MouseButtonPressed& mouse, State& state, sf::RenderWindow& window)
{
    ...
    if(state.game_ended || state.game_paused)
    {
        if(mouse.button == sf::Mouse::Button::Left){ //EX MODIFCA 
            ...
            else if(state.gs.exit_cb.cb_bounds.contains(mouse_pos)) state.exit(); //AGGIUNTA
        }
        return; 
    }
...
void handle_mouse_moved (const sf::Event::MouseMoved& ev, sf::RenderWindow& window, State& state)
{
    ...
    if(state.game_ended || state.game_paused){
        ...
        state.gs.exit_cb.mouse_focus = (state.gs.exit_cb.cb_bounds.contains(mouse_float_pos)? true : false);  //AGGIUNTA
        return; 
    }
    ...
    state.cp.exit.mouse_focus = (state.cp.exit.cb_bounds.contains(mouse_float_pos)? true : false); //AGGIUNTA
