...
float button_border_thickness = 5; //AGGIUNTA: spessore del bordo del bottone 
sf::Color button_color = sf::Color(192, 192, 192); //AGGIUNTA: colore di background dei pulsanti del control panel
sf::Color button_border_color = sf::Color::Black; //AGGIUNTA: colore del bordo dei bottoni
const float button_text_thickness = 1.5; //AGGIUNTA: spessore del testo dei pulsanti del Control Panel
const float button_text_proportion =3.5; //AGGIUNTA: proporzione tra testo all'interno dei pulsanti e pulsanti
...
//AGGIUNTA: struttura per rappresentare i vari pulsanti del gioco
struct Control_Button
{
    sf::Vector2f cb_pos; //posizione del pulsante
    sf::Vector2f cb_size; //dimensione del pulsante
    sf::Text cb_text; //testo all'interno del pulsante che andrà ad indicare in gioco al il tipo del pulsante
    sf::FloatRect cb_bounds; //confini del pulsante (posizione e dimensione)
    button_type cb_type; //tipo del pulsante
...
struct Control_Panel 
{
    ...
    //si è passato il bordo come parametro per i calcoli della dimensione e della posizione del control panel 
    Control_Panel(Border border): 
                                    cp_size({border.b_size.x -(border.thickness *2), border.b_size.y -(border.thickness *2)}), //il pannello presenta la stessa dimensione del pannello di gioco 
                                    cp_pos(window_horizontal_displacement + panel_thickness, border.b_pos.y + border.thickness), //il pannello viene posizionato alla sinistra del pannello di gioco e i due pannelli vengono messi con uguale distanziamento dal bordo della finestra 
                                    button_size({(cp_size.x-(control_button_horizontal_displacement*2))/3, (cp_size.y-(control_button_vertical_displacement*2))/8}),  //la dimensione del pulsante è un terzo della larghezza del pannello e un ottavo della lunghezza 
                                    new_game(button_type::new_game, {cp_pos.x + control_button_horizontal_displacement, cp_pos.y + control_button_vertical_displacement}, button_size), //creazione del pulsante di una nuova partita 
                                    pause(button_type::pause, {cp_pos.x +cp_size.x - control_button_horizontal_displacement - button_size.x, cp_pos.y + control_button_vertical_displacement}, button_size) //creazione del pulsante di pausa 
                                    {}
...
struct State 
{
    ...
    Stop_Panel stop_p; 
    Control_Panel cp; //AGGIUNTA: aggiunta del pannello di controllo
    int mouse_cell;
    bool focus; 
    bool first_move; 
    bool game_ended;
...
void Control_Button::draw (sf::RenderWindow& window){
    sf::RectangleShape cb (cb_size); //il pulsante è un rettangolo con la dimensione indicata precedentemente 
    cb.setPosition(cb_pos); //impostazione della posizione
    cb.setFillColor(button_color);  //impostazione colore sfondo
    cb.setOutlineThickness(button_border_thickness); //impostazione spessore bordo
    cb.setOutlineColor(button_border_color); //impostazione colore bordo
    window.draw(cb);

    //testo nel pulsante
    cb_text.setFont(font);
    cb_text.setCharacterSize(cb.getSize().y/button_text_proportion); //impostazione dimensione testo
    cb_text.setFillColor(text_color); //impostazione colore testo
    cb_text.setOutlineThickness(button_text_thickness); //impostazione spessore bordo testo
    cb_text.setOutlineColor(text_border_color); //impostazione colore bordo testo
    //impostazione del tipo di testo nel pulsante
    switch(cb_type){
...
void Control_Panel::draw (sf::RenderWindow& window){
    sf::RectangleShape cp(cp_size); //il control panel è un rettangolo della dimensione indicata precedentemente
    cp.setPosition(cp_pos); //impostazione della posizione 

    //presenta colori e spessore uguali allo Stop Panel
...
void State::draw (sf::RenderWindow& window){
    ...
    stop_p.draw(window);
    cp.draw(window); //AGGIUNTA: rappresentazione del Control Panel 
}
