...
enum class button_type{new_game, pause, easy, medium, hard, exit}; //AGGIUNTA

////////////////PANNELLO DI CONTROLLO////////////////

const float control_horizontal_displacement = 30; 
const float control_vertical_displacement = 30; 
const float control_gap = 5.f; //AGGIUNTA
...
struct Control_Panel 
{
    ...
    Control_Button pause, new_game, exit; //EX MODIFICA E AGGIUNTA
    ...
    Control_Panel(Border border): 
                                    ...
                                    exit(button_type::exit, {cp_pos.x +cp_size.x/2.f - button_size.x/2.f, pause.cb_pos.y + pause.cb_size.y + control_vertical_displacement + control_gap}, button_size) //AGGIUNTA 
                                    {}
...
{ 
    ...
    bool visible; 
    int time;
    Control_Button new_game_cb, easy_cb, medium_cb, hard_cb, exit_cb; //AGGIUNTA
    ...
    Game_Stop ():
                ...
                time(0), 
                new_game_cb(button_type::new_game,{gs_pos.x + gs_size.x/4.f - std_stop_button_width/2.f, gs_pos.y + gs_size.y - stop_gap - std_stop_button_heigth}, {std_stop_button_width, std_stop_button_heigth}), //MODIFICA 
                ...
                exit_cb(button_type::exit, {gs_pos.x + gs_size.x*3.f/4.f - std_stop_button_width/2.f, gs_pos.y + gs_size.y - stop_gap - std_stop_button_heigth}, {std_stop_button_width, std_stop_button_heigth}) //AGGIUNTA
                {} 
...
void Game_Stop::draw(sf::RenderWindow& window){ 
    ...
    { 
        ...
        new_game_cb.cb_pos = {gs_pos.x + gs_size.x/4.f - exit_cb.cb_size.x/2.f, gs_pos.y + gs_size.y - stop_gap - std_stop_button_heigth*1.5f}; //MODIFICA 
        new_game_cb.cb_bounds = {new_game_cb.cb_pos, new_game_cb.cb_size};
        //TOLTO new_game_cb.draw(window); 
        exit_cb.cb_size = new_game_cb.cb_size;  //AGGIUNTA
        exit_cb.cb_pos = {gs_pos.x + gs_size.x*3.f/4.f - exit_cb.cb_size.x/2.f, new_game_cb.cb_pos.y}; //AGGIUNTA
        exit_cb.cb_bounds = {exit_cb.cb_pos, exit_cb.cb_size}; //AGGIUNTA
    }  
    new_game_cb.draw(window); //EX MODIFICA 
    exit_cb.draw(window); //AGGIUNTA
}
...
void Control_Button::draw (sf::RenderWindow& window){
    ...
    switch(cb_type){
        ...
        case button_type::exit: //AGGIUNTA
...
void handle_mouse_pressed (const sf::Event::MouseButtonPressed& mouse, State& state, sf::RenderWindow& window)
{
    ...
    if(state.game_ended || state.game_paused)
    {
        ...
        if(state.gs.new_game_cb.cb_bounds.contains(mouse_pos) && mouse.button == sf::Mouse::Button::Left){ 
    ...
    if( mouse.button == sf::Mouse::Button::Left){
        if(state.sp.visible){ 
            ...
            {
                state.set_difficulty(Difficulty::easy); 
                state.sp.visible = false; //ex modifica
            }
