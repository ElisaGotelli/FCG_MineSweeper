...
////////////////FINESTRA////////////////
const char* window_title = "Graphic Exit";
...
enum class button_type{new_game, pause, exit, easy, medium, hard}; //MODIFICATO: aggiunta di un nuovo tipo di pulsante per consentire l'uscita dalla schermata di gioco e il ritorno alla schermata iniziale
...
const float control_vertical_gap = 10; //AGGIUNTA: gap verticale tra i vari pulsanti del control panel 
...
struct Control_Panel 
{
    ...
    Control_Button exit; //AGGIUNTA: pulsante per l'uscita dalla schermata di gioco
    ...
    Control_Panel(Border border): 
                                    ...
                                    exit(button_type::exit, {cp_pos.x +cp_size.x/2 - button_size.x/2, pause.cb_pos.y + pause.cb_size.y + control_button_vertical_displacement + control_vertical_gap/2}, button_size) //AGGIUNTA: il pulsante di uscita è posizionato nel Control Panel centralmente in larghezza e sotto agli altri due pulsanti di un valore fisso deciso precedentemente
                                    {}
...
struct Stop_Panel{ 
    ...
    bool visible;
    int time;
    Control_Button new_game_cb, easy_cb, medium_cb, hard_cb, exit_cb; //AGGIUNTA: aggiunto il pulsante per l'uscita dalla schermata di gioco che apparirà in tutti i tipi di Stop Panel tranne in quello di new game
    ...
    Stop_Panel (): 
                ...
                time(0), 
                new_game_cb(button_type::new_game,{stop_pos.x + stop_size.x/3 - stop_cb_width/2, stop_pos.y + stop_size.y - stop_gap - stop_cb_height}, {stop_cb_width, stop_cb_height}), //MODIFICATO: il pulsante di New Game non viene più messo centralmente in larghezza ma spostato a lato per poter avere lo spazio per il nuovo pulsante appena creato 
                ...
                exit_cb(button_type::exit, {stop_pos.x + stop_size.x*2/3 - stop_cb_width/2, stop_pos.y + stop_size.y - stop_gap - stop_cb_height}, {stop_cb_width, stop_cb_height})  //AGGIUNTA: il pulsante di uscita viene posizionato alla destra del pulsante di new game che era già presente nel codice delle tppe precedenti 
                {}
...
void Stop_Panel::draw(sf::RenderWindow& window){
    ...
    exit_cb.draw(window); //AGGIUNTA: rappresentazione del nuovo pulsante di uscita
}
...
void Control_Button::draw (sf::RenderWindow& window){
    ...
    switch(cb_type){
        ...
        case button_type::exit://AGGIUNTA: nuovo tipo di testo per rappresentare il pulsante di uscita dalla schermata di gioco
...
void Control_Panel::draw (sf::RenderWindow& window){
    ...
    pause.draw(window);
    exit.draw(window); //AGGIUNTA: rappresentazione del nuovo pulsante di uscita
}
